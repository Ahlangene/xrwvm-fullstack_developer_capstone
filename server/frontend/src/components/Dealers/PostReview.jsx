...
const PostReview = () => {
  ...
  const postreview = async () => {
    ...
    // Splits "Toyota Corolla" safely into Make and Model
    let model_split = model.split(" ");
    let make_chosen = model_split[0];
    let model_chosen = model_split.slice(1).join(" "); // Handles multi-word models like "C Class"
    ...
    // Handles various status returns from Django backend setups
  ...
  const get_cars = async () => {
    ...
    // Safely reads the exact casing returned by your views.py endpoint
    if (retobj.CarModels) {
      setCarmodels(Array.from(retobj.CarModels));
    } else if (retobj.car_models) {
